...
function drawScale(){
	...
	graphBoard_ctx.fillText("Générations", graphLongueur/2, graphHauteur - (graphScaleOffset/2 + textOffset));
	
	graphBoard_ctx.rotate(-(Math.PI/2));
	graphBoard_ctx.fillText("Nombre d'individus", -graphLongueur/2, graphScaleOffset/2 - textOffset);
	graphBoard_ctx.rotate(Math.PI/2);
