...
function drawPoint(context, x, y, size, color) {
	context.fillRect(x + (resolutionX/2), y + (resolutionY/2), size, size);
	context.fillStyle = color;
	context.fill();
}
...
function drawLine(context, x1, y1, x2, y2, color) {
	context.beginPath();
	context.moveTo(x1 + (resolutionX/2), y1 + (resolutionY/2));
    context.lineTo(x2 + (resolutionX/2), y2 + (resolutionY/2));
...
function populate(d, n) {
	for (var i = 0; i < n; i++) {
		d.push([getRandomInt(-resolutionX/2, resolutionX/2), getRandomInt(-resolutionY/2, resolutionY/2), 0])
	}
}

function calculNdraw(list) {
	for (var i = 0; i < list.length; i++) {
		list[i][2] = equation(list[i][0], list[i][1]);
		
		var blue = list[i][0]/resolutionX * 255
		var green = list[i][1]/resolutionY * 255
		var red = (list[i][2]/resolutionX + list[i][2]/resolutionY) / 2 * 255
		...
		drawLine(board_ctx, list[i][0], list[i][1], list[i][1], list[i][2], color)
	}
}

function equation(y, dy) {
	return dy;
}
