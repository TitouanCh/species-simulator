<div class="article">
    ...
    <div id='Species Simulator parameters' class='simulation_parameters'
        style="background-color: #333; color: white; padding: 20px; text-align: center; border-radius: 20px; margin-top: 12px;">
    ...
    <p>Particle life is a simulation that depicts life-like behavior arising from particles following a set of simple
        rules. Particle life reminds me a lot of John Conway’s game of life in that sense. However, the execution and
        mechanics of these two models differ significantly.</p>

    <h2> What are the rules of particle life?</h2>

    <p>Particle life is made up of many particles. You can see them flying around above. Each particle has the ability
        to either attract or repel other particles.</p>

    <p>How particles interact, if they repel or attract each other, is dependent on their color.
        At the start of the simulation, particles are randomly assigned a color, which represents their species. Next,
        each possible pair of colors (order matters) is assigned a random number. This random number will determine the
        nature of the interaction between the two colors: a negative number indicates an attractive force, while a
        positive number represents a repulsive force.
    </p>
    ...
    <p>The strength of particle interactions diminishes based on the distance between two particles. This distance is
        determined using a selected norm. Additionally, if two particles get too close, a strong repelling force is
        applied to prevent them from passing through each other.</p>

    <p>All these interactions can lead to endless energy creation in our simulation. We counteract this by adding a
        strong friction force to all particles, which prevents them from continuously accelerating.</p>
    ...
    <p>However, due to the O(n^2) complexity of each step in the simulation, as calculating the next position of each
        particle requires determining the resulting force from each other particle acting on it, we need to employ two
        methods to optimize the simulation.</p>

    <ul>
        <li>The first method is space partitioning, where we divide the simulation space into partitions. For each
            particle, we only need to calculate the interaction with particles from its partition and neighboring
            partitions.</li>

        <li>The second method is multithreading which is built on top of space partitioning. We can assign different
            partitions to different threads. The performance benefit of this method varies because serializing the data
            and sending it to the threads each frame proves to be quite expensive.</li>
    </ul>
</div>