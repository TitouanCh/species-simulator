...
species_simulation.species_data = [
  [10, 30, "#7c9ac2", 5, [false, false]],
  [10, 20, "#293e58", 5, [false, false]],
];
species_simulation.species_data_cached =
  species_simulation.species_data.slice();
...
species_simulation.offset_height =
  (species_simulation.board.height % species_simulation.cell_height) / 2;
species_simulation.offset_width =
  (species_simulation.board.width % species_simulation.cell_width) / 2;
species_simulation.predation = 2;
species_simulation.ageMax = true;
species_simulation.ratios = [
  species_simulation.canvas_width,
  species_simulation.canvas_height,
  species_simulation.cell_width,
  species_simulation.cell_height,
];
...
function species_start() {
  ...
  this.species_data.forEach((species) => {
    if (!this.once) {
      ...
      species[0] = parseInt(
        document.getElementById("tauxReproductionEspece" + idx).value
      );
      species[3] = parseInt(
        document.getElementById("tauxPredationEspece" + idx).value
      );
      species[1] = parseInt(
        document.getElementById("ageMaxEspece" + idx).value
      );
  ...
  setupWorker = new Worker("/assets/js/species-simulator/species_worker.js");
  ...
  setupWorker.onmessage = (event) => {
    ...
    this.grid_height =
      (this.ratios[1] - this.offset_height * 2) / this.ratios[3];
...
function species_draw() {
  ...
  this.grid.forEach((item) => {
    ...
    this.board_ctx.fillRect(
      xoffset[0] + item.x * this.ratios[2],
      xoffset[1] + item.y * this.ratios[3],
      this.ratios[2],
      this.ratios[3]
    );
  });
    ...
    if (item.espece > 0) {
      ...
      if (this.predation) {
        item.voisins.forEach((voisin) => {
          ...
          if (
            this.grid[voisin].espece > 0 &&
            this.species_data_cached[this.grid[voisin].espece - 1][4][
              item.espece - 1
            ]
          ) {
            var b = getRandomInt(1, 100);
...
function showSpecies() {
  ...
  this.species_data.forEach((ispecies) => {
    string +=
      '<div style="margin-top: 29px; grid-column: ' +
      (idx % 3) +
      "; grid-row: " +
      (Math.floor((idx - 1) / 3) + 1) +
      ';">';
    string +=
      '<div style="font-style: italic;">Specie ' +
      idx +
    ...
    string +=
      ...
      '">';
    string +=
      '<label class ="hiddenLabel" for="couleurEspece' +
      idx +
      "\">Sélectionner la couleur de l'espèce " +
      idx +
      ".</label>";

    string +=
      "</br>Probability of Reproduction: " +
      '<input type="number" min="1" max="100" value="' +
      ispecies[0] +
      '" id="tauxReproductionEspece' +
      idx +
      '">%';
    string +=
      '<label class ="hiddenLabel" for="tauxReproductionEspece' +
      idx +
      '">Choose a reproduction percentage for the specie ' +
      idx +
      ".</label>";

    string +=
      "</br>Probability of Predation: " +
      '<input type="number" min="1" max="100" value="' +
      ispecies[3] +
      '" id="tauxPredationEspece' +
      idx +
      '">%';
    string +=
      '<label class ="hiddenLabel" for="tauxPredationEspece' +
      idx +
      '">Choose a predation percentage for the specie ' +
      idx +
      ".</label>";
    ...
    this.species_data.forEach((jspecies) => {
      string +=
        '<label for="regimeEspece' +
        idx +
        "for" +
        idx2 +
        '" style ="visibility: visible; display: inline;">Specie ' +
        idx2 +
        ' </label><input id="regimeEspece' +
        idx +
        "for" +
        idx2 +
        '" type="checkbox"/>';
    ...
    string +=
      ...
      '">';
    string +=
      '<label class ="hiddenLabel" for="ageMaxEspece' +
      idx +
      '">Maximum age for specie ' +
      idx +
      ".</label>";
    string += "</div>";
