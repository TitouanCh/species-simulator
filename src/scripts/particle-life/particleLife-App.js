...
function ready() {
	...
	resolutionX = document.getElementById("enviroWidth").value;
	resolutionY = document.getElementById("enviroHeight").value;
	
	board.width = resolutionX;
	board.height = resolutionY;
	
	G = parseFloat(document.getElementById("G").value);
	dragCoef = parseFloat(document.getElementById("drag").value);
	
	if (document.getElementById("nbEspece").value <= 8) {
		numSpecies = document.getElementById("nbEspece").value;
	} else {
		numSpecies = 8;
	...
	populate(data, document.getElementById("nbIndividus").value, numSpecies);
}
...
function restart() {
	data = [] // [specie, x, y, vx, vy, nx, ny]
	forceBook = []
	massBook = []
	colorPalette = ["#597d64", "#e8094a", "#315465", "#7b118b", "#02aacf", "#bfe6c7", "#6e4358", "#305557"] // DYNAMITE
