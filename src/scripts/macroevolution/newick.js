...
function ready() {
	if (document.getElementById('treeString') != null) {
		treeString = document.getElementById('treeString').value;
		tree = str2tree(treeString);
...
function main() {
	...
	if (!(document.getElementById('pause').checked)){
		...
		drawTree(board_ctx, resolutionX * 0.95, resolutionY, tree);
...
function drawTree(ctx, w, h, tree, offsetx = 0, offsety = 0, marg_height = 10, marg_side = 40) {
	var weightsList = [];
	var totalChildren = 0;
	...
	for (let i = 0; i < tree.length; i++) {
		...
		if (circles || base_x + x >= resolutionX * 0.9) { drawPoint(ctx, base_x + x, y + space_sum, 5, "black"); }
		// console.log((base_x + x)/resolutionX);
